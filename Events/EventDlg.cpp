...
DWORD WINAPI Thread1(LPVOID lp)
{
	...
	for(int i = 0; i < 5; i++)//захват ресурса только 5 раз
	{
		...
		ResetEvent(hEvent); // перевод события в несигнальное состояние. Явно возвращаем ресурс (флажок) Ручной сброс
	}
	return 0;
}
...
DWORD WINAPI Thread2(LPVOID lp)
{
	...
	for(int i = 0; i < 5; i++)
	{
		...
		}//Автомтический сброс нет резетивента
	}
	return 0;
}
...
void CEventDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	switch(id)
	{
		case IDC_BUTTON1:
			{
				CreateEvent(NULL, TRUE /* ручной сброс события */, FALSE /* несигнальное состояние (изначально не запущено,по клику ничего не происходит и обьект в свободном состоянии) */, TEXT("{2BA7C99B-D9F7-4485-BB3F-E4735FFEF139}"));
		...
		case IDC_BUTTON2:
			{
				...
				SetEvent(h); // перевод события в сигнальное состояние =реализуем захват ресурса (флажочка) меняем с FALSE на TRUE состоянии обьекта и потоки начинают сразу работать
			}
			break;
		case IDC_BUTTON3:
			{
				...
				EnableWindow(GetDlgItem(hwnd, IDC_BUTTON3), FALSE);//первый сразу запускается ,автоматический сброс
				HANDLE h;
		...
		case IDC_BUTTON4:
			{
				...
				SetEvent(h); // перевод события в сигнальное состояние =можем реализовать захват ресурса (флажка)
			}
			break;
	}
}
